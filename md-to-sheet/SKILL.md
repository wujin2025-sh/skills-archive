---
name: md-to-sheet
description: 将Obsidian需求MD文件批量写入腾讯文档在线表格。读取指定目录下的需求MD文件，解析需求名称、描述、涉及系统等字段， 去重后追加到腾讯文档表格中，自动处理超链接、多系统换行、背景色等格式。
  触发词：MD写入表格、需求导入表格、MD到腾讯文档、@tdoc配合MD文件、追加需求到表格。 当用户同时提供腾讯文档表格引用（@tdoc）和本地MD文件路径，并要求将MD内容写入表格时触发。
disable: false
---
...
2. MD文件命名格式（两种均支持）：
   - 旧格式：`YYYYMMDD-需求-【系统名】需求标题.md` / `YYYYMMDD-需求-需求标题.md`
   - **新格式（推荐，ba-to-dev 命名规范）**：`YYYYMMDD-R26xxxxxxx-纯中文标题.md`，如 `20260901-R2608070169-个微两融柜台保证金计算公式改造针对个微两融.md`
3. 解析每个MD文件，提取以下字段：
...
- 涉及系统：**严格优先从【评审纪要】中的「2. 改造范围」、「8. 联测涉及系统」以及「3. 各系统改造功能」标题提取**，严禁使用单字或泛词（如"参数"、"清算"、"交易系统"）进行全文粗暴包含匹配。**关键：多个系统名必须用 `\n` 换行符连接，每个系统独占一行。严禁使用空格、顿号(、)、逗号(,) 或空格分隔符连接。**正确示例：`"低延时\n集中清算"`
- 关联系统：如内容中提到"低延时已切换"等关联关系，提取为关联系统。多系统同样用 `\n` 换行符连接
- 计划排期：兼容匹配 `预计上线日期/预计完成时间/预计上线时间/计划上线：2026年XX月`，提取为 `X月份`
- 评审状态：**第一次写入（追加新行）时，默认值固定为「待评审」**；更新已有记录时保留表格现有的评审状态。
...
1. **有则更新**：若在线表格已存在同名需求，获取其所在行号。写入时仅覆盖更新提取到的动态属性（提出日期、描述、涉及系统、关联系统、排期、超链接），但**保留**已有的“优先级(B列)”、“story拆分(I列)”、“评审状态(J列)”、“备注(K列)”和“参会人员(L列)”字段，避免覆盖人工修改与审核结果。
2. **无则追加**：若不存在，则在表格第一个检测到的全空行位置追加写入新行，其中**评审状态固定写入默认值「待评审」**。
...
5. **流水线闭环提醒（强制要求）**：每次 `/md-to-sheet` 执行完毕并在回复用户时，**必须在回复末尾明确附带下一步工作流提示**：
   > 💡 **下一步工作流提示**：在线表格中完成评审（完善备注、参会人员并将评审状态置为 `通过`）后，可随时运行 `/sheet-to-md` 将表格最新内容反写回本地 Markdown 需求文档。
