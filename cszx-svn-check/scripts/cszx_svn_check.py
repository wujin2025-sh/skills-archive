...
# 默认仓库配置
DEFAULT_UAT_URL = "https://jyjs.svn.gtja.net/svn/jjywpt/Src/01Branches/02Trunk_jygl_prod/jygl_uat_2024"
DEFAULT_PROD_URL = "https://jyjs.svn.gtja.net/svn/jjywpt/Src/01Branches/02Trunk_jygl_prod/jygl_prod_s"
...
def main():
    parser = argparse.ArgumentParser(description="CSZX 参数中心 SVN 需求号/任务号分支差异检查工具")
    ...
    print("=" * 75)
    print(f"CSZX SVN 差异并发检查启动时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    report_file = os.path.join(Config.OUTPUT_DIR, f"cszx_svn_check_report_{ts}.md")
    with open(report_file, "w", encoding="utf-8") as f:
        f.write(f"# CSZX SVN 分支差异检查报告\n\n")
    ...
    summary_file = os.path.join(Config.OUTPUT_DIR, f"cszx_svn_check_summary_{ts}.txt")
    with open(summary_file, "w", encoding="utf-8") as f:
        f.write(f"CSZX SVN CHECK SUMMARY - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write("=" * 70 + "\n")
