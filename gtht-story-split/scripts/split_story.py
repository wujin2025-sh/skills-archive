...
# 内置默认映射：系统名 → 模板名（当系统名与模板名不一致时自动匹配）
DEFAULT_TEMPLATE_MAP = {
    "低延时交易系统": "低延时信创域",
    "集中交易系统": "集中交易-两融",                # 两融需求默认
    "参数管理中心后台": "参数中心后台",
    "证金监管数据报送子系统": "证金监管数据报送子系统",
    "集中清算系统": "集中清算系统",
}
...
def parse_template_map(s):
    ...
    自动合并 DEFAULT_TEMPLATE_MAP，命令行参数优先级高于默认映射。
    """
    out = dict(DEFAULT_TEMPLATE_MAP)  # 以内置映射为底
    if not s:
        return out
    for item in s.split(","):
        item = item.strip()
...
async def open_new_story_drawer(page):
    ...
    if await btn.count() == 0:
        ...
        try:
            img_path = "/Users/wujin/.gemini/antigravity/brain/56a9e868-7947-4afc-b827-58d0f032a9f2/scratch/split_error.png"
            await page.screenshot(path=img_path)
            print(f"  📸 已成功保存截图至: {img_path}")
...
def main():
    ...
    # ---------- API 快速路径（默认全面向 API 直调）：Story 拆分 + 发起评估 + 非开发工作量 ----------
    if not args.browser and not args.tags:
        import platform_api
        ...
        # 仅发起评估模式
        if args.assess_only:
            res_assess = platform_api.initiate_assessment(args.req_id, dry_run=args.dry_run)
            print(f"[api] ✅ 发起技术评估成功: {json.dumps(res_assess, ensure_ascii=False)}")
            print(f"[api] 完成，耗时 {__import__('time').time()-t0:.2f}s")
            return
        ...
        need_biz = "是" if dept != "技术研发部" else "否" if args.need_biz_accept == "auto" else args.need_biz_accept
        ...
        for idx, sys_item in enumerate(systems):
            ...
            if args.dry_run:
                ...
                ok_count += 1
                continue
            print(f"[api] ✅ {sys_name}（{sys_type}）Story 创建并绑定成功: Code={res.get('storyCode')}")
            ok_count += 1
        ...
        print(f"[api] 全流程完成，总耗时 {__import__('time').time()-t0:.2f}s")
        return

    # 运行异步浏览器逻辑（仅在显式 --browser 或包含 --tags 且无 API 接口时兜底）
...
async def run_browser(args):
    ...
    try:
        ...
        # 自动受理检测与执行
        btn_accept = page.locator('button:has-text("受理需求")').first
        if await btn_accept.count() > 0:
            print("  发现「受理需求」按钮，正在自动执行受理...")
            # 1. 注册 native dialog 自动确认
            page.once("dialog", lambda dialog: dialog.accept())
            # 2. 点击受理按钮
            await btn_accept.click()
            await page.wait_for_timeout(1000)
            # 3. 检查是否有 Antd 的 popover 或 modal 确认按钮并点击
            confirm_selectors = [
