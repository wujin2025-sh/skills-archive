...
| `brew install --cask drawio` fails with "already an App at..." | Delete the manual install first: `rm -rf /Applications/draw.io.app` and then retry the `brew install` command. |
| macOS CLI execution fails with `Unable to find helper app` | This happens when executing `/Applications/draw.io.app/Contents/MacOS/draw.io` directly or via a symlink, as Electron gets confused about the app path. Fix it by replacing the symlink with a shell wrapper script: `#!/bin/bash \n exec "/Applications/draw.io.app/Contents/MacOS/draw.io" "$@"` |
| Opening exported SVG/PNG in draw.io Desktop fails with "Not a diagram file" (非绘图文件) | You forgot the `-e` flag during export (which embeds the original diagram XML model metadata). Re-export by adding `-e` (e.g. `drawio -x -f svg -e -o diagram.svg diagram.drawio`). |
