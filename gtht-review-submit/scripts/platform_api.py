...
def save_story(demand_id: str, system_name: str, story_type: str = "改造",
               ...
               dry_run: bool = False) -> dict:
    """新建 Story（POST /story-service/stcpstory/storydetail/create）。

    ⚠️ 实现状态：payload 由「道合大宗模板内容 + Story 数据模型」推断构造，
    尚未经真实提交逐字段校验 —— 调用方应在失败时回退 Playwright（split_story --browser）。

    template: get_template_by_system 返回项；未提供时按 system_name 自动查找。
    story_type: 改造 | 配合测试（配合测试在名称前加【配合测试】）。
    need_biz_accept: 是/否（默认按需求提交人部门：非技术研发部→是）。
    biz_acceptor: 业务验收人（姓名-工号 或 工号）。
    parent_desc: 父需求描述（HTML，复制父需求描述后内容）。
    """
    info = get_demand_info(demand_id)
    ...
    if need_biz_accept == "是":
        biz_acceptor = biz_acceptor or c.get("businessAccept") or ""
    ...
    return SESSION.api_post("/story-service/stcpstory/storydetail/create", payload)
