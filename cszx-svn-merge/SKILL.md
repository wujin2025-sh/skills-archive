...
此技能能够自动化执行按Task编号维度的分支代码合并、冲突自动回撤、以及无冲突自动提交。本工具**不依赖特定操作系统，全面支持 Windows 与 macOS**，并且会自动根据当前操作系统适配其默认的本地工作目录 and SVN 可执行文件路径。
...
5. **冲突处理**：
 ...
 6. **报告汇总**：处理完所有任务后，在指定的输出目录下生成：
    * `summary_<时间戳>.txt`：整体执行 of 分类统计与明细。
...
| `--local-dir` | `-l` | `/Volumes/Macintosh HD_Data/Project/jygl/jygl_sys` | 本地 SVN 工作副本目录路径。在 Windows 上默认为 `F:\spb\spbsrc`，在 macOS 上默认为 `/Volumes/Macintosh HD_Data/Project/jygl/jygl_sys`。 |
| `--merge-source` | `-s` | `https://jyjs.svn.gtja.net/svn/jjywpt/Src/01Branches/02Trunk_jygl_prod/jygl_uat_2024` | 来源分支的 SVN 仓库 URL（例如 UAT 分支）。 |
...
2. **询问/推导配置**：
   ...
   * **工作副本目录**（`-l`）：这里是参数中心默认的 `/Volumes/Macintosh HD_Data/Project/jygl/jygl_sys`。
3. **构造并执行命令**：
...
```bash
# 方式 1：通过参数直接传入数据文本
python3 /Users/wujin/.workbuddy/skills/cszx-svn-merge/scripts/cszx_svn_merge.py \
  -p "703" \
  -d "JZJYPT-T202504823	123456	债券质押式开发需求	蔡静雯-123576"

# 方式 2：使用 stdin 传入多行 Excel 复制的数据
cat << 'EOF' | python3 /Users/wujin/.workbuddy/skills/cszx-svn-merge/scripts/cszx_svn_merge.py -p "703"
...
```bash
python3 cszx_svn_merge.py ... -u "" --svn-password ""
```
...
```bash
/Users/wujin/.workbuddy/binaries/python/envs/default/bin/python cszx_svn_merge.py ...
```
...
/Users/wujin/.workbuddy/binaries/python/envs/default/bin/python \
  /Users/wujin/.workbuddy/skills/cszx-svn-merge/scripts/cszx_svn_merge.py \
  -d $'JZJYPT-T202606586\t任务标题\t经办人-工号' \
  -l "/Volumes/Macintosh HD_Data/Project/jygl/jygl_sys" \
...
当代码合并过程中检测到冲突并自动回撤后，我们可以运行冲突邮件草稿生成脚本，将冲突的文件明细和代码比对详情直接发送（起草）给对应的开发人员。本技能**只发送冲突及比对明细，不发送版本邮件**。

### 执行命令

```bash
# 自动读取当前目录的 '合并版本号.txt' 作为版本目录，在 Coremail 起草冲突提示邮件
python3 /Users/wujin/.workbuddy/skills/cszx-svn-merge/scripts/draft_conflict_emails.py
