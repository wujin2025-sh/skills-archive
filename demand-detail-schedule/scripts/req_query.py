    ...
    if os.path.exists(p):
        try:
            with open(p, "r", encoding="utf-8") as f:
                ...
                # 如果是占位符，忽略它
                if u in ("YOUR_USERNAME", "你的工号"):
                    u = ""
                if p_plain in ("YOUR_PASSWORD", "YOUR_PLAIN_PASSWORD", "你的登录密码", "密码", "您的密码"):
                    p_plain = ""
...
if not USERNAME or not PASSWORD:
    print("\n" + "=" * 75, file=sys.stderr)
    print("❌ [配置缺失错误] 未在 config.json 中配置有效的用户名与密码！", file=sys.stderr)
    print("-" * 75, file=sys.stderr)
    print("💡 在线登录金融科技平台拉取最新数据需要有效的工号与密码。", file=sys.stderr)
    print(f"👉 请编辑技能目录下的 config.json 配置文件：", file=sys.stderr)
    print(f"   配置文件路径: {os.path.join(SKILL_ROOT, 'config.json')}", file=sys.stderr)
    print("   标准配置格式:", file=sys.stderr)
    print("   {", file=sys.stderr)
    print('     "username": "你的工号",', file=sys.stderr)
    print('     "password": "你的登录密码",', file=sys.stderr)
    print('     "platform_url": "http://fintech.gtht.com.cn"', file=sys.stderr)
    print("   }", file=sys.stderr)
    print("=" * 75 + "\n", file=sys.stderr)
